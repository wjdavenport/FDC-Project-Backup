...
def main():
    ...
    print(export_text(tree2, feature_names=list(text_feat.columns)))
        ...
        seed_df : pd.DataFrame
            Merged seed dataframe with pmid/title/abstract/label.
        text_feat : pd.DataFrame
            Output of boolean_text_features(seed_df)
        proba : np.ndarray
            Model probabilities (for FULL seed set, i.e., lr.predict_proba(X)[:,1])
        n : int
        ...
        # Random sample (stratified, simple)
