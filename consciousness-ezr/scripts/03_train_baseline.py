...
def export_unlabeled_sample_for_audit(
    df_all: pd.DataFrame,
    seed_pmids: pd.Series,
    model: LogisticRegression,
    tfidf: TfidfVectorizer,
    n: int = 200,
    outpath: str = "unlabeled_model_sample.csv",
):
    """
    Export a random sample of UNLABELED references for external audit.

    - Excludes all PMIDs that appear in the seed label set.
    - Uses the trained TF-IDF + LR model to compute:
        * model_prob (P(class 1, "relevant"))
        * pred (hard prediction at threshold 0.5)
        * abs_margin (|prob - 0.5|, a simple confidence measure)
    - Writes a CSV with:
        pmid, title, abstract, model_prob, pred, abs_margin
    """

    # Exclude already-labeled PMIDs
    mask_unlabeled = ~df_all["pmid"].isin(seed_pmids)
    df_unlab = df_all.loc[mask_unlabeled].copy()

    if df_unlab.empty:
        print("[unlabeled audit] No unlabeled records available.")
        return

    # Build features using the same pipeline as training
    X_text_unlab = build_text(df_unlab)
    X_meta_unlab = build_meta(df_unlab)

    X_t_unlab = tfidf.transform(X_text_unlab)
    X_unlab = hstack([X_t_unlab, X_meta_unlab])

    # Model probabilities for class 1
    proba_unlab = model.predict_proba(X_unlab)[:, 1]

    # Assemble output table
    df_out = df_unlab[["pmid", "title", "abstract"]].reset_index(drop=True)
    df_out["model_prob"] = proba_unlab
    df_out["pred"] = (df_out["model_prob"] >= 0.5).astype(int)
    df_out["abs_margin"] = (df_out["model_prob"] - 0.5).abs()

    # Random sample of size n (or all, if fewer)
    df_sample = df_out.sample(
        n=min(n, len(df_out)),
        random_state=1073,
    )
    ...
    print(
        f"[unlabeled audit] Exported {len(df_sample)} unlabeled rows → {outpath}"
    )
